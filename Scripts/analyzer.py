...
if not os.path.exists(analyzer_results_dir):
	sys.exit("error: Static Anaylzer results not found, expected in {0!s}".format(analyzer_results_dir))
...
for result in glob.iglob(os.path.join(analyzer_results_dir, "*.plist")):
	with open(result) as f:
		...
		for diagnostic in plist['diagnostics']:
			...
			print "{0!s}:{1!s}:{2!s}: error: Static Anaylzer Issue: {3!s}".format(path, location['line'], location['col'], diagnostic['description'])
			exit_code = 1
